/*
 * Archivo de datos para fuentes
 * Formato: Vertical, LSB arriba.
 * Cada caracter ASCII (32-126) ocupa (Ancho * ((Alto+7)/8)) bytes.
 * Los bytes para cada columna están consecutivos.
 *
 * Fuente 7x10:
 * Cada columna tiene 10 bits. Se usan 2 bytes por columna.
 * El primer byte tiene bits 0-7 (LSB a MSB -> pixel de arriba a abajo).
 * El segundo byte tiene bits 8-9 (en sus posiciones 0 y 1).
 * Total bytes por caracter = Ancho * BytesPorColumna = 7 * 2 = 14 bytes.
 *
 * Fuente obtenida/adaptada de:
 * https://github.com/afiskon/stm32-ssd1306/blob/master/ssd1306/fonts.c
 * Autor Original: Aleksander Alekseev (afiskon)
 * Licencia: MIT
 */
#include "fonts.h"

// --- Font 7x10 --- (Caracteres 32 a 126)
// Total caracteres: 126 - 32 + 1 = 95
// Tamaño total: 95 caracteres * 14 bytes/caracter = 1330 bytes
const uint8_t Font7x10[1330] = {
    // Space (ASCII 32)
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Lower 8 bits
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // Upper 2 bits
    // ! (ASCII 33)
    ...
    0x00, 0x01, 0x03, 0x01, 0x00, 0x00, 0x00,
    // 1 (ASCII 49)
    ...
    0x01, 0x03, 0x01, 0x01, 0x00, 0x00, 0x00,
    // 4 (ASCII 52)
    ...
    0x00, 0x03, 0x01, 0x03, 0x00, 0x00, 0x00,
    // 9 (ASCII 57)
    0x06, 0x49, 0x49, 0x29, 0x1E, 0x00, 0x00,
    0x00, 0x03, 0x01, 0x01, 0x00, 0x00, 0x00,
    // : (ASCII 58)
    ...
    0x01, 0x03, 0x03, 0x00, 0x00, 0x00, 0x00,
    // K (ASCII 75)
    0x7F, 0x08, 0x14, 0x22, 0x41, 0x00, 0x00,
    0x03, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00,
    // L (ASCII 76)
    0x7F, 0x40, 0x40, 0x40, 0x40, 0x00, 0x00,
    0x03, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00,
    // M (ASCII 77)
    ...
    0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
    // b (ASCII 98)
    0x7F, 0x44, 0x44, 0x44, 0x38, 0x00, 0x00,
    0x03, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
    // c (ASCII 99)
    0x38, 0x44, 0x44, 0x44, 0x44, 0x00, 0x00,
    0x00, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00,
    // d (ASCII 100)
    0x38, 0x44, 0x44, 0x44, 0x7F, 0x00, 0x00,
    0x00, 0x01, 0x01, 0x01, 0x03, 0x00, 0x00,
    // e (ASCII 101)
    ...
    // g (ASCII 103)
    0x08, 0x54, 0x54, 0x54, 0x3C, 0x00, 0x00,
    0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00,
    // h (ASCII 104)
    0x7F, 0x08, 0x04, 0x04, 0x78, 0x00, 0x00,
    0x03, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00,
    // i (ASCII 105)
    ...
    0x03, 0x00, 0x01, 0x03, 0x00, 0x00, 0x00,
    // l (ASCII 108)
    0x00, 0x41, 0x7F, 0x40, 0x00, 0x00, 0x00,
    0x00, 0x03, 0x03, 0x01, 0x00, 0x00, 0x00,
    // m (ASCII 109)
    0x7C, 0x04, 0x18, 0x04, 0x78, 0x00, 0x00,
    0x03, 0x01, 0x00, 0x01, 0x03, 0x00, 0x00,
    // n (ASCII 110)
    ...
    0x00, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00,
    // ~ (ASCII 126)
    0x08, 0x04, 0x08, 0x04, 0x08, 0x00, 0x00,
